.class public Landroid/taobao/windvane/view/WebNaviBar;
.super Landroid/taobao/windvane/view/AbstractNaviBar;
.source "SourceFile"


# annotations
.annotation runtime Ljava/lang/Deprecated;
.end annotation


# static fields
.field private static final INDEX_NEXT:I = 0x3

.field private static final INDEX_PRE:I = 0x2

.field private static final INDEX_REFRESH:I = 0x4


# instance fields
.field private drawableNext:[Landroid/graphics/drawable/Drawable;

.field private drawablePre:Landroid/graphics/drawable/Drawable;

.field private drawableRefresh:[Landroid/graphics/drawable/Drawable;

.field private imgNext:Landroid/widget/ImageButton;

.field private imgPre:Landroid/widget/ImageButton;

.field private imgRefresh:Landroid/widget/ImageButton;

.field private mContext:Landroid/content/Context;

.field private mHeight:I

.field private paint:Landroid/graphics/Paint;

.field private sideMargin:I

.field public webview:Landroid/webkit/WebView;


# direct methods
.method public constructor <init>(Landroid/content/Context;Landroid/util/AttributeSet;ILandroid/webkit/WebView;)V
    .locals 1

    .line 1
    invoke-direct {p0, p1, p2, p3}, Landroid/taobao/windvane/view/AbstractNaviBar;-><init>(Landroid/content/Context;Landroid/util/AttributeSet;I)V

    const/16 p2, 0x46

    .line 2
    iput p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->sideMargin:I

    const/16 p2, 0x64

    .line 3
    iput p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    const/4 p2, 0x0

    .line 4
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    .line 5
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    .line 6
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    const/4 p3, 0x2

    new-array v0, p3, [Landroid/graphics/drawable/Drawable;

    .line 7
    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    new-array p3, p3, [Landroid/graphics/drawable/Drawable;

    .line 8
    iput-object p3, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    .line 9
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawablePre:Landroid/graphics/drawable/Drawable;

    .line 10
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->mContext:Landroid/content/Context;

    .line 11
    iput-object p4, p0, Landroid/taobao/windvane/view/WebNaviBar;->webview:Landroid/webkit/WebView;

    .line 12
    invoke-virtual {p0, p1}, Landroid/taobao/windvane/view/WebNaviBar;->init(Landroid/content/Context;)V

    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Landroid/util/AttributeSet;Landroid/webkit/WebView;)V
    .locals 2

    .line 13
    invoke-direct {p0, p1, p2}, Landroid/taobao/windvane/view/AbstractNaviBar;-><init>(Landroid/content/Context;Landroid/util/AttributeSet;)V

    const/16 p2, 0x46

    .line 14
    iput p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->sideMargin:I

    const/16 p2, 0x64

    .line 15
    iput p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    const/4 p2, 0x0

    .line 16
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    .line 17
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    .line 18
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    const/4 v0, 0x2

    new-array v1, v0, [Landroid/graphics/drawable/Drawable;

    .line 19
    iput-object v1, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    new-array v0, v0, [Landroid/graphics/drawable/Drawable;

    .line 20
    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    .line 21
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawablePre:Landroid/graphics/drawable/Drawable;

    .line 22
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->mContext:Landroid/content/Context;

    .line 23
    iput-object p3, p0, Landroid/taobao/windvane/view/WebNaviBar;->webview:Landroid/webkit/WebView;

    .line 24
    invoke-virtual {p0, p1}, Landroid/taobao/windvane/view/WebNaviBar;->init(Landroid/content/Context;)V

    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Landroid/webkit/WebView;)V
    .locals 3

    .line 25
    invoke-direct {p0, p1}, Landroid/taobao/windvane/view/AbstractNaviBar;-><init>(Landroid/content/Context;)V

    const/16 v0, 0x46

    .line 26
    iput v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->sideMargin:I

    const/16 v0, 0x64

    .line 27
    iput v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    const/4 v0, 0x0

    .line 28
    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    .line 29
    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    .line 30
    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    const/4 v1, 0x2

    new-array v2, v1, [Landroid/graphics/drawable/Drawable;

    .line 31
    iput-object v2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    new-array v1, v1, [Landroid/graphics/drawable/Drawable;

    .line 32
    iput-object v1, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    .line 33
    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawablePre:Landroid/graphics/drawable/Drawable;

    .line 34
    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->mContext:Landroid/content/Context;

    .line 35
    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->webview:Landroid/webkit/WebView;

    .line 36
    invoke-virtual {p0, p1}, Landroid/taobao/windvane/view/WebNaviBar;->init(Landroid/content/Context;)V

    return-void
.end method

.method public static synthetic access$000(Landroid/taobao/windvane/view/WebNaviBar;)Landroid/widget/ImageButton;
    .locals 0

    .line 1
    iget-object p0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    return-object p0
.end method

.method public static synthetic access$100(Landroid/taobao/windvane/view/WebNaviBar;)[Landroid/graphics/drawable/Drawable;
    .locals 0

    .line 1
    iget-object p0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    return-object p0
.end method

.method public static synthetic access$200(Landroid/taobao/windvane/view/WebNaviBar;)Landroid/content/Context;
    .locals 0

    .line 1
    iget-object p0, p0, Landroid/taobao/windvane/view/WebNaviBar;->mContext:Landroid/content/Context;

    return-object p0
.end method

.method private setClickListener(Landroid/widget/ImageButton;I)V
    .locals 1

    .line 1
    new-instance v0, Landroid/taobao/windvane/view/WebNaviBar$1;

    invoke-direct {v0, p0, p2}, Landroid/taobao/windvane/view/WebNaviBar$1;-><init>(Landroid/taobao/windvane/view/WebNaviBar;I)V

    invoke-virtual {p1, v0}, Landroid/widget/ImageButton;->setOnClickListener(Landroid/view/View$OnClickListener;)V

    return-void
.end method

.method private setNextIcon(Landroid/content/Context;Landroid/content/res/Resources;)V
    .locals 3

    .line 1
    new-instance v0, Landroid/widget/ImageButton;

    invoke-direct {v0, p1}, Landroid/widget/ImageButton;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    const/16 p1, 0x71

    .line 2
    invoke-virtual {v0, p1}, Landroid/widget/ImageButton;->setId(I)V

    .line 3
    iget-object p1, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    const v0, -0xa0a0b

    invoke-virtual {p1, v0}, Landroid/widget/ImageButton;->setBackgroundColor(I)V

    .line 4
    new-instance p1, Landroid/widget/RelativeLayout$LayoutParams;

    iget v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    add-int/lit8 v1, v0, -0x2

    invoke-direct {p1, v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;-><init>(II)V

    const/16 v0, 0xf

    .line 5
    invoke-virtual {p1, v0}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    const/4 v0, 0x5

    const/16 v1, 0x70

    .line 6
    invoke-virtual {p1, v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(II)V

    .line 7
    iget v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->sideMargin:I

    mul-int/lit8 v0, v0, 0x2

    iput v0, p1, Landroid/widget/RelativeLayout$LayoutParams;->leftMargin:I

    .line 8
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    const-string v1, ""

    invoke-static {p2, v1}, Landroid/taobao/windvane/util/ImageTool;->toDrawable(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v1

    const/4 v2, 0x0

    aput-object v1, v0, v2

    .line 9
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    const-string v1, ""

    invoke-static {p2, v1}, Landroid/taobao/windvane/util/ImageTool;->toDrawable(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object p2

    const/4 v1, 0x1

    aput-object p2, v0, v1

    .line 10
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    aget-object v0, v0, v2

    invoke-virtual {p2, v0}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 11
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    sget-object v0, Landroid/widget/ImageView$ScaleType;->CENTER_INSIDE:Landroid/widget/ImageView$ScaleType;

    invoke-virtual {p2, v0}, Landroid/widget/ImageButton;->setScaleType(Landroid/widget/ImageView$ScaleType;)V

    .line 12
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    const/4 v0, 0x3

    invoke-direct {p0, p2, v0}, Landroid/taobao/windvane/view/WebNaviBar;->setClickListener(Landroid/widget/ImageButton;I)V

    .line 13
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    invoke-virtual {p0, p2, p1}, Landroid/widget/RelativeLayout;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    return-void
.end method

.method private setPreIcon(Landroid/content/Context;Landroid/content/res/Resources;)V
    .locals 2

    .line 1
    new-instance v0, Landroid/widget/ImageButton;

    invoke-direct {v0, p1}, Landroid/widget/ImageButton;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    const/16 p1, 0x70

    .line 2
    invoke-virtual {v0, p1}, Landroid/widget/ImageButton;->setId(I)V

    .line 3
    iget-object p1, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    const v0, -0xa0a0b

    invoke-virtual {p1, v0}, Landroid/widget/ImageButton;->setBackgroundColor(I)V

    .line 4
    new-instance p1, Landroid/widget/RelativeLayout$LayoutParams;

    iget v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    add-int/lit8 v1, v0, -0x2

    invoke-direct {p1, v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;-><init>(II)V

    const/16 v0, 0xf

    .line 5
    invoke-virtual {p1, v0}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    const/16 v0, 0x9

    .line 6
    invoke-virtual {p1, v0}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    .line 7
    iget v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->sideMargin:I

    iput v0, p1, Landroid/widget/RelativeLayout$LayoutParams;->leftMargin:I

    const-string v0, ""

    .line 8
    invoke-static {p2, v0}, Landroid/taobao/windvane/util/ImageTool;->toDrawable(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object p2

    iput-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawablePre:Landroid/graphics/drawable/Drawable;

    .line 9
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    invoke-virtual {v0, p2}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 10
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    sget-object v0, Landroid/widget/ImageView$ScaleType;->CENTER_INSIDE:Landroid/widget/ImageView$ScaleType;

    invoke-virtual {p2, v0}, Landroid/widget/ImageButton;->setScaleType(Landroid/widget/ImageView$ScaleType;)V

    .line 11
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    const/4 v0, 0x2

    invoke-direct {p0, p2, v0}, Landroid/taobao/windvane/view/WebNaviBar;->setClickListener(Landroid/widget/ImageButton;I)V

    .line 12
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    invoke-virtual {p0, p2, p1}, Landroid/widget/RelativeLayout;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    return-void
.end method

.method private setRefreshIcon(Landroid/content/Context;Landroid/content/res/Resources;)V
    .locals 3

    .line 1
    new-instance v0, Landroid/widget/ImageButton;

    invoke-direct {v0, p1}, Landroid/widget/ImageButton;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    const/16 p1, 0x72

    .line 2
    invoke-virtual {v0, p1}, Landroid/widget/ImageButton;->setId(I)V

    .line 3
    iget-object p1, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    const v0, -0xa0a0b

    invoke-virtual {p1, v0}, Landroid/widget/ImageButton;->setBackgroundColor(I)V

    .line 4
    new-instance p1, Landroid/widget/RelativeLayout$LayoutParams;

    iget v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    add-int/lit8 v1, v0, -0x2

    invoke-direct {p1, v0, v1}, Landroid/widget/RelativeLayout$LayoutParams;-><init>(II)V

    const/16 v0, 0xf

    .line 5
    invoke-virtual {p1, v0}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    const/16 v0, 0xb

    .line 6
    invoke-virtual {p1, v0}, Landroid/widget/RelativeLayout$LayoutParams;->addRule(I)V

    .line 7
    iget v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->sideMargin:I

    iput v0, p1, Landroid/widget/RelativeLayout$LayoutParams;->rightMargin:I

    .line 8
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    const-string v1, ""

    invoke-static {p2, v1}, Landroid/taobao/windvane/util/ImageTool;->toDrawable(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object v1

    const/4 v2, 0x0

    aput-object v1, v0, v2

    .line 9
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    const-string v1, ""

    invoke-static {p2, v1}, Landroid/taobao/windvane/util/ImageTool;->toDrawable(Landroid/content/res/Resources;Ljava/lang/String;)Landroid/graphics/drawable/Drawable;

    move-result-object p2

    const/4 v1, 0x1

    aput-object p2, v0, v1

    .line 10
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    aget-object v0, v0, v2

    invoke-virtual {p2, v0}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 11
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    sget-object v0, Landroid/widget/ImageView$ScaleType;->CENTER_INSIDE:Landroid/widget/ImageView$ScaleType;

    invoke-virtual {p2, v0}, Landroid/widget/ImageButton;->setScaleType(Landroid/widget/ImageView$ScaleType;)V

    .line 12
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    const/4 v0, 0x4

    invoke-direct {p0, p2, v0}, Landroid/taobao/windvane/view/WebNaviBar;->setClickListener(Landroid/widget/ImageButton;I)V

    .line 13
    iget-object p2, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    invoke-virtual {p0, p2, p1}, Landroid/widget/RelativeLayout;->addView(Landroid/view/View;Landroid/view/ViewGroup$LayoutParams;)V

    return-void
.end method


# virtual methods
.method public init(Landroid/content/Context;)V
    .locals 4

    .line 1
    iput-object p1, p0, Landroid/taobao/windvane/view/WebNaviBar;->mContext:Landroid/content/Context;

    const/16 v0, 0x6e

    .line 2
    invoke-virtual {p0, v0}, Landroid/widget/RelativeLayout;->setId(I)V

    const v0, -0xa0a0b

    .line 3
    invoke-virtual {p0, v0}, Landroid/widget/RelativeLayout;->setBackgroundColor(I)V

    const/4 v0, 0x1

    .line 4
    invoke-virtual {p0, v0}, Landroid/widget/RelativeLayout;->setClickable(Z)V

    .line 5
    invoke-virtual {p1}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object v0

    invoke-virtual {v0}, Landroid/content/res/Resources;->getDisplayMetrics()Landroid/util/DisplayMetrics;

    move-result-object v0

    iget v0, v0, Landroid/util/DisplayMetrics;->density:F

    const/high16 v1, 0x42500000    # 52.0f

    mul-float v1, v1, v0

    const/high16 v2, 0x3f000000    # 0.5f

    add-float/2addr v1, v2

    float-to-int v1, v1

    .line 6
    iput v1, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    .line 7
    new-instance v1, Landroid/view/ViewGroup$LayoutParams;

    iget v2, p0, Landroid/taobao/windvane/view/WebNaviBar;->mHeight:I

    const/4 v3, -0x1

    invoke-direct {v1, v3, v2}, Landroid/view/ViewGroup$LayoutParams;-><init>(II)V

    .line 8
    invoke-virtual {p0, v1}, Landroid/widget/RelativeLayout;->setLayoutParams(Landroid/view/ViewGroup$LayoutParams;)V

    .line 9
    new-instance v1, Landroid/graphics/Paint;

    invoke-direct {v1}, Landroid/graphics/Paint;-><init>()V

    iput-object v1, p0, Landroid/taobao/windvane/view/WebNaviBar;->paint:Landroid/graphics/Paint;

    const v2, -0x333334

    .line 10
    invoke-virtual {v1, v2}, Landroid/graphics/Paint;->setColor(I)V

    const/high16 v1, 0x41c80000    # 25.0f

    mul-float v0, v0, v1

    float-to-int v0, v0

    .line 11
    iput v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->sideMargin:I

    .line 12
    invoke-virtual {p1}, Landroid/content/Context;->getResources()Landroid/content/res/Resources;

    move-result-object v0

    .line 13
    invoke-direct {p0, p1, v0}, Landroid/taobao/windvane/view/WebNaviBar;->setRefreshIcon(Landroid/content/Context;Landroid/content/res/Resources;)V

    .line 14
    invoke-direct {p0, p1, v0}, Landroid/taobao/windvane/view/WebNaviBar;->setNextIcon(Landroid/content/Context;Landroid/content/res/Resources;)V

    .line 15
    invoke-direct {p0, p1, v0}, Landroid/taobao/windvane/view/WebNaviBar;->setPreIcon(Landroid/content/Context;Landroid/content/res/Resources;)V

    return-void
.end method

.method public onDraw(Landroid/graphics/Canvas;)V
    .locals 7

    .line 1
    invoke-super {p0, p1}, Landroid/widget/RelativeLayout;->onDraw(Landroid/graphics/Canvas;)V

    .line 2
    invoke-virtual {p0}, Landroid/widget/RelativeLayout;->getWidth()I

    move-result v0

    int-to-float v4, v0

    iget-object v6, p0, Landroid/taobao/windvane/view/WebNaviBar;->paint:Landroid/graphics/Paint;

    const/4 v2, 0x0

    const/4 v3, 0x0

    const/4 v5, 0x0

    move-object v1, p1

    invoke-virtual/range {v1 .. v6}, Landroid/graphics/Canvas;->drawLine(FFFFLandroid/graphics/Paint;)V

    return-void
.end method

.method public resetState()V
    .locals 4

    .line 1
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    const/4 v1, 0x1

    if-eqz v0, :cond_1

    .line 2
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->webview:Landroid/webkit/WebView;

    invoke-virtual {v0}, Landroid/webkit/WebView;->canGoForward()Z

    move-result v0

    if-eqz v0, :cond_0

    .line 3
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    iget-object v2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    aget-object v2, v2, v1

    invoke-virtual {v0, v2}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    goto :goto_0

    .line 4
    :cond_0
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgNext:Landroid/widget/ImageButton;

    iget-object v2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableNext:[Landroid/graphics/drawable/Drawable;

    const/4 v3, 0x0

    aget-object v2, v2, v3

    invoke-virtual {v0, v2}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 5
    :cond_1
    :goto_0
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgPre:Landroid/widget/ImageButton;

    if-eqz v0, :cond_2

    .line 6
    iget-object v2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawablePre:Landroid/graphics/drawable/Drawable;

    invoke-virtual {v0, v2}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    .line 7
    :cond_2
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    if-eqz v0, :cond_3

    .line 8
    iget-object v2, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    aget-object v1, v2, v1

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    :cond_3
    return-void
.end method

.method public startLoading()V
    .locals 3

    .line 1
    iget-object v0, p0, Landroid/taobao/windvane/view/WebNaviBar;->imgRefresh:Landroid/widget/ImageButton;

    if-eqz v0, :cond_0

    .line 2
    iget-object v1, p0, Landroid/taobao/windvane/view/WebNaviBar;->drawableRefresh:[Landroid/graphics/drawable/Drawable;

    const/4 v2, 0x0

    aget-object v1, v1, v2

    invoke-virtual {v0, v1}, Landroid/widget/ImageButton;->setImageDrawable(Landroid/graphics/drawable/Drawable;)V

    :cond_0
    return-void
.end method

.method public stopLoading()V
    .locals 0

    return-void
.end method
