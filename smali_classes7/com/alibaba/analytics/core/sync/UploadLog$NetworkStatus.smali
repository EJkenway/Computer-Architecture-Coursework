.class public final enum Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;
.super Ljava/lang/Enum;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lcom/alibaba/analytics/core/sync/UploadLog;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x4019
    name = "NetworkStatus"
.end annotation

.annotation system Ldalvik/annotation/Signature;
    value = {
        "Ljava/lang/Enum<",
        "Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;",
        ">;"
    }
.end annotation


# static fields
.field private static final synthetic $VALUES:[Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

.field public static final enum ALL:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

.field public static final enum FOUR_GENERATION:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

.field public static final enum NONE:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

.field public static final enum THRID_GENERATION:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

.field public static final enum TWO_GENERATION:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

.field public static final enum WIFI:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;


# direct methods
.method public static constructor <clinit>()V
    .locals 13

    .line 1
    new-instance v0, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    const-string v1, "ALL"

    const/4 v2, 0x0

    invoke-direct {v0, v1, v2}, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->ALL:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    new-instance v1, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    const-string v3, "WIFI"

    const/4 v4, 0x1

    invoke-direct {v1, v3, v4}, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;-><init>(Ljava/lang/String;I)V

    sput-object v1, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->WIFI:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    new-instance v3, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    const-string v5, "TWO_GENERATION"

    const/4 v6, 0x2

    invoke-direct {v3, v5, v6}, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;-><init>(Ljava/lang/String;I)V

    sput-object v3, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->TWO_GENERATION:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    new-instance v5, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    const-string v7, "THRID_GENERATION"

    const/4 v8, 0x3

    invoke-direct {v5, v7, v8}, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;-><init>(Ljava/lang/String;I)V

    sput-object v5, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->THRID_GENERATION:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    new-instance v7, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    const-string v9, "FOUR_GENERATION"

    const/4 v10, 0x4

    invoke-direct {v7, v9, v10}, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;-><init>(Ljava/lang/String;I)V

    sput-object v7, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->FOUR_GENERATION:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    new-instance v9, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    const-string v11, "NONE"

    const/4 v12, 0x5

    invoke-direct {v9, v11, v12}, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;-><init>(Ljava/lang/String;I)V

    sput-object v9, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->NONE:Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    const/4 v11, 0x6

    new-array v11, v11, [Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    aput-object v0, v11, v2

    aput-object v1, v11, v4

    aput-object v3, v11, v6

    aput-object v5, v11, v8

    aput-object v7, v11, v10

    aput-object v9, v11, v12

    .line 2
    sput-object v11, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->$VALUES:[Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    return-void
.end method

.method private constructor <init>(Ljava/lang/String;I)V
    .locals 0

    .line 1
    invoke-direct {p0, p1, p2}, Ljava/lang/Enum;-><init>(Ljava/lang/String;I)V

    return-void
.end method

.method public static valueOf(Ljava/lang/String;)Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;
    .locals 1

    .line 1
    const-class v0, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    invoke-static {v0, p0}, Ljava/lang/Enum;->valueOf(Ljava/lang/Class;Ljava/lang/String;)Ljava/lang/Enum;

    move-result-object p0

    check-cast p0, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    return-object p0
.end method

.method public static values()[Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;
    .locals 1

    .line 1
    sget-object v0, Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->$VALUES:[Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    invoke-virtual {v0}, [Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;->clone()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, [Lcom/alibaba/analytics/core/sync/UploadLog$NetworkStatus;

    return-object v0
.end method
